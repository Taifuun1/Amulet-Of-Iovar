...
func createOutpostAreas():
	...
	for _y in range(Globals.gridSize.y):
		grid[outpostArea[1].x - 2][_y].tile = Globals.tiles.WALL_DUNGEON
	
	for _y in range(Globals.gridSize.y):
		grid[outpostArea[2].x + 2][_y].tile = Globals.tiles.WALL_DUNGEON
	
	for _x in range(outpostArea[1].x - 1, outpostArea[2].x + 2):
		grid[_x][0].tile = Globals.tiles.WALL_DUNGEON
	
	for _x in range(outpostArea[1].x - 1, outpostArea[2].x + 2):
		grid[_x][Globals.gridSize.y - 1].tile = Globals.tiles.WALL_DUNGEON
					...
					for _areaSizeX in range(_x, _x + _areaSize.x + 1):
						for _areaSizeY in range(_y, _y + _areaSize.y + 1):
							if (
								...
								grid[_areaSizeX][_areaSizeY].tile = Globals.tiles.WALL_DUNGEON
							else:
								grid[_areaSizeX][_areaSizeY].tile = Globals.tiles.FLOOR_DUNGEON
...
func getAreaXSideLength(_x, _y, maxLength):
	...
	for _areaX in range(_x, _x + maxLength):
		if grid[_areaX][_y].tile == Globals.tiles.WALL_DUNGEON or grid[_areaX][_y].tile == Globals.tiles.SIDEWALK or length == maxLength:
			return length
		length += 1
	return length
...
func getAreaYSideLength(_x, _y, maxLength):
	...
	for _areaY in range(_y, _y + maxLength):
		length += 1
		if grid[_x][_areaY].tile == Globals.tiles.WALL_DUNGEON or grid[_x][_areaY].tile == Globals.tiles.SIDEWALK or grid[_x][_areaY].tile == Globals.tiles.FLOOR_DUNGEON or length == 8:
			if _areaY == 22 or grid[_x][_areaY].tile == Globals.tiles.WALL_DUNGEON or grid[_x][_areaY].tile == Globals.tiles.SIDEWALK:
				length -= 1
			return length
	return length
...
func createCaveAreas():
	var _side = randi() % 2
	createCaveArea(_side, Globals.tiles.UP_STAIR_DUNGEON)
	if _side == 1:
		createCaveArea(0, Globals.tiles.DOWN_STAIR_DUNGEON)
	else:
		createCaveArea(1, Globals.tiles.DOWN_STAIR_DUNGEON)
...
func placeStair2(_availableStairFloors, _stair):
	...
	elif _stair == Globals.tiles.DOWN_STAIR_DUNGEON:
		stairs.downStair = _randomFloor

func placeDoors2(room, _side):
	var placement
	rooms[_side].append([])
